...
async fn main() {
    ...
    let console_app_secret:oauth2::ConsoleApplicationSecret = serde_json::from_reader(file).unwrap();
    ...
    let r = hub.files().list().param("fields","files(id,name,mimeType,ownedByMe)").doit().await.unwrap().1;
    if r.next_page_token.is_some() {
        panic!("PAGINATION NOT YET IMPLEMENTED!")
    }
    for f in r.files.as_ref().unwrap() {
        println!("{} '{}' {} {}", f.id.as_ref().unwrap(), f.name.as_ref().unwrap(),f.mime_type.as_ref().unwrap(), f.owned_by_me.unwrap());
    }

    
    for f in r.files.as_ref().unwrap() {
        if f.mime_type.as_ref().unwrap() == "application/vnd.google-apps.document" && f.owned_by_me.unwrap() {
            let v = hub.files().export(&f.id.as_ref().unwrap(), "application/vnd.oasis.opendocument.text").doit().await.unwrap();
            ...
            println!("downloaded '{}' as '{}' ({} bytes)", f.name.as_ref().unwrap(), out_name, content.len());
